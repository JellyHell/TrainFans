...
public class StudentsClassListFragment extends BaseFragment implements Observer<ArticleList> {
    ...
    protected void initView() {
        ...
        mRefreshableRecyclerView.getRecyclerview().setSwipeMenuCreator((swipeLeftMenu, swipeRightMenu, viewType) -> {
            SwipeMenuItem deleteItem = new SwipeMenuItem(getActivity())
                    ...
                    .setHeight(AppUtil.dip2px(getActivity(),65));
        ...
        mRefreshableRecyclerView.getRecyclerview().setSwipeMenuItemClickListener((closeable, adapterPosition, menuPosition, direction) -> {
            ...
            mStudentClassList.get(adapterPosition/2).mClassRoom.clear();
    ...
    public void onNext(ArticleList articleList) {
        if(mPage == 1){
            mStudentClassList.clear();
        }

        mStudentClassList.add(new StudentClass());
        mStudentClassList.add(new StudentClass());
        mStudentClassList.add(new StudentClass());
...
//            mAdapter.canLoadMore(true);
    ...
    private void setApapter(){
        if(mAdapter == null){
            mAdapter = new StudentClassAdapter(getActivity(),mStudentClassList);
//            mAdapter.setOnItemClickListener((adapterView, view, i, l) -> {});
            mRefreshableRecyclerView.getRecyclerview().setAdapter(mAdapter);
        }else {
            mAdapter.notifyDataSetChanged();
        }
    }
