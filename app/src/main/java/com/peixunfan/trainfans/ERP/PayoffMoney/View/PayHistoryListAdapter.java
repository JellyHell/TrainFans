...
public class PayHistoryListAdapter  extends
        SectionedRecyclerViewAdapter<SectionHeaderHolder,TeacherMoneyCountViewHolder,SignUpFooterHolder,RecyclerView.ViewHolder> {

    private Context mContext;

    public ArrayList<Article> mRecords = new ArrayList<>();

    public PayHistoryListAdapter(Context context,ArrayList<Article> records) {
        mContext = context;
    ...
    @Override
    protected TeacherMoneyCountViewHolder onCreateItemViewHolder(View arg0) {
    ...
    @Override
    protected void onBindItemViewHolder(final TeacherMoneyCountViewHolder holder, int section, int position) {
        //Cell信息显示
        holder.itemView.setOnClickListener(v -> mItemClickListener.onItemClick(null,null,position,section));
